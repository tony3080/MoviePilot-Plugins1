...
`v0.8.2` 使用独立固定色值渲染卡片标签和文件大小，选中前后保持固定边框宽度，长自定义标签按 `@` 在卡片内换行；来源按钮增加 RSS hash、qB comment 和 tracker/torrent ID 多重回退。只有勾选“入库”且下载完成的 RSS 项目才写入入库管理，切换页面和筛选条件会清空卡片选择。插件继续保留彼此独立的 qB 源名称处理与 MoviePilot 硬链接目标命名：
...
当前版本会执行 RSS 任务配置的 qB 入队和源文件/目录改名写操作，并生成、展示和持久化独立的 MP 硬链接目标映射；尚未实际创建或删除硬链接，不调用 CloudDrive2 API，也不会切换外部追更或扫库开关。
