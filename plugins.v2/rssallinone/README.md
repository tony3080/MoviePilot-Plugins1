...
`v0.6.0` 已实现 RSS 任务配置持久化、RSS CRON、手动立即执行和 qB 入队。后续存在两套独立命名：RSS 任务勾选“重命名”时按任务规则实际修改 qB 文件/目录，“添加中文标题”也可独立修改 qB 名称；硬链接入库时则再按 MoviePilot 命名模板生成目标文件名。下载完成后的 qB 重命名执行、识别、库存复核、硬链接入库和删除任务仍属于后续阶段。删除时使用持久化的 qB hash/文件索引/当前源路径与硬链接路径映射，CloudDrive2 不由插件调用 API。
