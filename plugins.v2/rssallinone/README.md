...
`v0.12.1` 调整文件管理识别交互：刷新按钮旁的“批量识别”逐项处理当前打开目录下的子文件夹和媒体文件，并在后台显示进度；每个文件夹或文件条目右侧的“识别”只处理该条目。两个操作点击后直接执行，不再弹出浏览器确认框。文件夹单项识别会递归收集媒体文件，文件单项识别只处理当前文件。识别前仍同时核对源路径和持久化源文件映射，避免生成重复卡片。
