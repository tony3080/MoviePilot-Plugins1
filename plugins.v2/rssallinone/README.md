...
`v0.4.2` 在可运行框架上完成了可编辑 RSS 任务、受任务范围约束的 qB 只读同步、本地 STRM 库存核对和目录规划：
...
`v0.4.2` 已实现 RSS 任务配置的持久化、QB 管理范围联动和本地 STRM 库存判断，并修复库存目录标题反向提取时重复追加年份的问题；尚未实现 RSS 拉取、种子推送和“立即执行”，因此页面不会提供虚假的执行按钮。
