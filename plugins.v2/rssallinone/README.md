...
`v0.4.3` 在可运行框架上完成了可编辑 RSS 任务、受任务范围约束的 qB 只读同步、本地 STRM 库存核对和目录规划：
...
`v0.4.3` 已实现 RSS 任务配置的持久化、QB 管理范围联动、本地 STRM 库存判断和 MoviePilot 文件/任务双来源资源元数据命名，并提供逐任务资源识别诊断；尚未实现 RSS 拉取、种子推送和“立即执行”，因此页面不会提供虚假的执行按钮。
