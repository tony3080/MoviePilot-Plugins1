...
`v0.13.26` 优化批次收尾与卡片状态。

`v0.13.25` 优化任务回调与批次操作。
...
`v0.13.16` 优化媒体管理与清理体验。

`v0.13.15` 增强后台处理的异常识别与回退保护。

`v0.13.14` 优化后台处理的路径兼容性与异常恢复。

`v0.13.13` 优化媒体处理稳定性与页面展示。

`v0.13.12` 增强后台处理稳定性。

`v0.13.11` 清理无效配置并加强任务归属和删除处理。

`v0.13.10` 完善 RSS 任务启动调度与后台任务显示。

`v0.13.9` 优化任务并行处理。

`v0.13.8` 增强异常恢复、通知校验、删除保护与数据安全。

`v0.13.7` 修复页面切换时的数据刷新问题。

`v0.13.6` 优化记录清理与历史数据维护。

`v0.13.5` 优化界面文字、导航和图标。

`v0.13.4` 优化任务操作和外部状态控制。

`v0.13.3` 优化路径处理和多路径兼容。
