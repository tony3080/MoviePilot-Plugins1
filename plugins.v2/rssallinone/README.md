...
`v0.6.0` 已实现 RSS 任务配置持久化、RSS CRON、手动立即执行和 qB 入队。下载完成后的识别、库存复核、硬链接入库和删除任务仍属于后续阶段；硬链接目标文件名会按 MoviePilot 命名规则生成，但 qB 下载源文件和目录保持原名原路径。删除时使用持久化的源路径/硬链接路径映射，CloudDrive2 不由插件调用 API，硬链接建立后由其现有备份监听接管。
