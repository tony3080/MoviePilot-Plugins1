...
`v0.4.4` 在可运行框架上完成了可编辑 RSS 任务、受任务范围约束的 qB 只读同步、本地 STRM 库存核对和目录规划：
...
`v0.4.4` 已实现 RSS 任务配置的持久化、QB 管理范围联动、本地 STRM 库存判断和 MoviePilot 文件/任务双来源资源元数据命名，并修正 `customization` 的实时识别与模板传递。添加中文标题属于后续 qB 重命名流程，不参与当前库存目标命名；国配/特效若已存在于任务或文件名中，会按 MoviePilot 自定义占位符进入 `customization`，固定归位动作留待 qB 重命名执行阶段。尚未实现 RSS 拉取、种子推送和“立即执行”。
