...
`v0.10.1` 拆分入库管理刷新语义：已入库卡片不再调用 qB 和 MoviePilot 重新识别，而是直接使用持久化的 `new_rel`、`inventory_path` 和 TMDB ID 逐文件复查本地 STRM 库存；未入库、待入库和回退卡片仍执行重新识别与库存对比。这样 CD2/STRM 逐步生成时，已入库卡片可以准确显示 `1/20`、`2/20` 等实时库存进度。
