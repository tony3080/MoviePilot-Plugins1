...
`v0.13.7` 修复入库管理与 QB 管理之间切换时短暂显示上一页卡片的问题。页面切换会立即清空旧列表并进入加载状态，同时忽略已经过期的异步接口响应。

`v0.13.6` 后台任务页增加“清除已结束任务”；qB 任务实际删除后，对应 RSS 历史会从页面消失并压缩为限量保留的去重指纹，避免再次推送；已入库卡片执行“删除硬链接和源文件”后，会同步清理媒体记录、文件映射、qB 快照和已完成删除计划。

`v0.13.5` 中文化总览能力、运行阶段及后台任务类型和状态，并在后台任务中直接展示结果或首个错误；文件管理移动到入库管理前方，左侧入口、插件卡片和插件页统一使用龙形图标。

`v0.13.4` 将追更与 SA 扫库状态灯移到插件主页面的“后台任务”标签后方，可直接读取和切换状态。QB 管理新增多选“删除任务”，只操作当前插件已展示的受管 qB 卡片，后端固定保留已下载文件；成功后立即移除对应卡片，失败项保留以便重试。

`v0.13.3` 移除单一 staging 根目录配置。插件自动收集所有启用源路径路由中的硬链接根目录，按本次硬链接路径进行最长前缀匹配；`/MP/电影UP`、`/MP/剧集UP` 和 `/SSD/云盘/l` 等多个本地根目录可以共用一个 CD2 上传任务目标根目录。

`v0.13.2` 修正外部联动配置的程序边界：追更控制独立使用 Emby 地址、PageId 和 Token，扫库控制独立使用 SA 地址、账号、密码、配置名和节点名；回调字段按后端实际校验标明必填关系，CD2 与回调技术参数收进高级设置。

`v0.13.1` 扩展 Emby 扫库完成回调的正文兼容性，支持对象、数组、JSON 字符串和表单文本，避免 Emby 测试通知因正文不是 JSON 对象而返回 422。
