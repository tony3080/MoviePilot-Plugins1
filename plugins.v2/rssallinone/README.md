...
`v0.9.0` 将 QB 管理改为纯当前任务视图：已从 qB 消失的任务直接删除快照，下载完成后也从 QB 管理移除；勾选“入库”的任务转入入库管理，未勾选的任务只记录完成标记。新增 qB 完成回调、入库管理按 RSS 任务名称多选筛选，以及任务级实时硬链接；实时硬链接会保留原 qB 源路径，把入库卡片的工作源切换到硬链接目录，并继续携带 P 来源信息。
...
当前版本会执行 RSS 任务配置的 qB 入队和源文件/目录改名写操作，并生成、展示和持久化独立的 MP 硬链接目标映射。只有 RSS 任务显式启用“完成后创建实时硬链接”时，插件才会在配置的源根目录与目标根目录之间立即创建同盘硬链接；普通入库硬链接和删除流程仍未开放。不调用 CloudDrive2 API，也不会切换外部追更或扫库开关。
