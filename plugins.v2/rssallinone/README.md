...
`v0.10.0` 接通入库管理顶部批量操作。在完成 RSS、qB 回调、MoviePilot 命名和逐文件库存判断后，可将已识别项目转为待入库，也可立即按持久化映射创建同盘硬链接。执行时只处理 `inventory_exists=false` 的文件；任一文件失败会删除本轮已经创建的硬链接并把错误写回卡片。已入库项目支持只删除插件实际创建的硬链接，或同时删除这些硬链接和映射中的源文件；不根据名称猜路径，也不调用 CloudDrive2 API。
...
双阶段命名当前已经落地：第一阶段在 RSS 入队后修改 qB 实际文件/目录名称；第二阶段使用 MoviePilot 当前命名规则生成独立的硬链接与 STRM 目标，并保存源目标映射。国语/特效站点标签解析、qB 完成回调、手动批量硬链接入库和基于映射的双侧删除已经接通；待入库 CRON、外部追更开关和扫库联动仍属于后续阶段。CloudDrive2 不由插件调用 API。
