...
| 当前插件版本 | `0.3.1` |
| 最近公开基线提交 | `60181bd34f0e989b0864ff831facfa570add1a93`（0.3.0） |
...
| `tests/test_lifecycle.py` | 隔离宿主依赖，模拟完成暂停、豆瓣总集数复核、正常完成、全历史搜索和永久去重 |
...
| `/history` | GET | bearer | 返回最近运行摘要、最近 50 条处理历史和受管订阅 |
...
当前使用 MoviePilot 默认 Vuetify JSON 页面，展示最近 50 条历史，字段包括：
...
1. 在测试 MoviePilot 中安装 `0.3.1`，使用一条 RSS 做端到端联调。
...
当前工作区版本是 v0.3.1。不要把 TMDB 集数写入 MoviePilot 订阅总集数；已开播但豆瓣总集数未知时可临时使用 100，并必须按执行周期复核回写。所有受管总集数均使用 manual_total_episode=1 锁定。保留完成后暂停、到期复核豆瓣、总集数未增加时正常完成的状态机和永久去重索引，不要修改用户自行创建的订阅。先报告你对当前实现、未验证部分和准备修改范围的理解，再开始改代码。修改后运行 compileall、全部 unittest 和 git diff --check。
```

## 20. 交接结论

当前代码已形成 0.3.1 工作区版本，包含地区过滤、未知总集数临时 100 集订阅及周期回写、订阅完成后暂停和豆瓣总集数复核状态机、最近 50 条展示、全历史搜索及永久去重索引。核心纯逻辑与静态契约测试已完成，但尚未发布，也尚未在真实 MoviePilot 宿主验证临时总集数回写、完成事件拦截和豆瓣复核。下一阶段应先用单条 RSS 和短确认天数完成端到端回归，再决定是否推送发布。
