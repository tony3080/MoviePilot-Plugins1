...
1. 在测试 MoviePilot 中安装 `0.5.0`，先验证 Vue 配置页、详情页和猫眼榜单，再使用一条 RSS 做端到端联调。
...
当前工作区版本是 v0.5.0。RSS 与猫眼全网电视剧/网剧榜单共用同一处理链；不要加入腾讯、爱奇艺、芒果或优酷的平台专榜。不要把 TMDB 集数写入 MoviePilot 订阅总集数；已开播但豆瓣总集数未知时可临时使用 100，并必须按执行周期复核回写。所有受管总集数均使用 manual_total_episode=1 锁定。保留活动订阅与订阅历史双重查重、完成后暂停与到期复核、永久去重索引、TMDB 查询诊断和“篇章名 → TMDB 分季名”匹配。修改后运行前端构建、compileall、全部 unittest 和 git diff --check。
```

## 20. 交接结论

当前代码已形成 0.5.0 版本。在 0.4.1 基础上增加猫眼全网电视剧和网剧榜单来源，猫眼条目继续复用豆瓣总集数、地区过滤、TMDB 匹配、订阅历史查重和集数锁定；Vue 详情页恢复右下角设置切换，并将处理记录、受管订阅和今日补齐状态中文化。自动测试和前端生产构建已完成，但仍需在真实 MoviePilot v2.15.0 宿主验证猫眼 Cookie 获取、榜单字段和设置切换事件。
