...
1. 在测试 MoviePilot 中安装 `0.4.1`，先验证 Vue 配置页和详情页，再使用一条 RSS 做端到端联调。
...
当前工作区版本是 v0.4.1。不要把 TMDB 集数写入 MoviePilot 订阅总集数；已开播但豆瓣总集数未知时可临时使用 100，并必须按执行周期复核回写。所有受管总集数均使用 manual_total_episode=1 锁定。保留活动订阅与订阅历史双重查重、完成后暂停与到期复核、永久去重索引、TMDB 查询诊断和“篇章名 → TMDB 分季名”匹配。每日订阅补齐只比较当日 08:00 快照和晚间进度，不跨日累积。修改后运行前端构建、compileall、全部 unittest 和 git diff --check。
```

## 20. 交接结论

当前代码已形成 0.4.1 版本。在 0.4.0 基础上增加了 MoviePilot 订阅历史同季查重、Vue 历史搜索与结果筛选、失败条目单独重试、TMDB 查询诊断与异常退避，以及“主剧名 + XXXX篇”对 TMDB 分季名的确定性匹配。自动测试和前端生产构建已完成，但仍需在真实 MoviePilot v2.15.0 宿主验证 Vue 远程组件加载、真实 TMDB 分季名称结构和单条重试接口。
