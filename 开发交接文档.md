...
当前工作区版本是 v0.5.1。RSS 与猫眼全网电视剧/网剧榜单共用同一处理链；豆瓣查询统一至少间隔 3 秒，触发限流时必须停止本批并保留未处理条目到下次执行。不要加入腾讯、爱奇艺、芒果或优酷的平台专榜。不要把 TMDB 集数写入 MoviePilot 订阅总集数；已开播但豆瓣总集数未知时可临时使用 100，并必须按执行周期复核回写。所有受管总集数均使用 manual_total_episode=1 锁定。保留活动订阅与订阅历史双重查重、完成后暂停与到期复核、永久去重索引、TMDB 查询诊断和“篇章名 → TMDB 分季名”匹配。修改后运行前端构建、compileall、全部 unittest 和 git diff --check。
```

## 20. 交接结论

当前代码已形成 0.5.1 版本。在 0.5.0 基础上修复猫眼批量查询触发豆瓣限流后被误记为“未找到”的问题：所有豆瓣调用至少间隔 3 秒，限流单独记录并停止本批，剩余条目留待下次执行；猫眼标题增加去引号及无年份降级查询。自动测试和前端生产构建已完成，但仍需在真实 MoviePilot v2.15.0 宿主验证豆瓣限流异常透传、猫眼 Cookie 获取、榜单字段和设置切换事件。
