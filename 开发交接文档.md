...
| 当前插件版本 | `0.2.0` |
| 功能基线提交 | `570d46125bf1dbf7834faa7436778a8fe7e21854` |
| 最低 MoviePilot 版本 | `>=2.15.1` |
| Python 测试版本 | 3.11 |
...
flowchart TD
    ...
    L --> M{最高分和领先分是否达标}
    M -->|否| N[记录低分或歧义，不创建订阅]
    M -->|是| O{相同 TMDB 与季度订阅是否已存在}
    O -->|是| P[记录 existing，不修改原订阅]
    O -->|否| Q[创建 MoviePilot 订阅]
    Q --> R[写入豆瓣总集数和手动锁定标记]
    R --> S[回读数据库验证锁定]
    S --> T[记录成功或锁定失败]
```
...
当前代码要求 MoviePilot `>=2.15.1`。该版本的订阅模型包含：
...
这是当前的保守策略。后续如果要支持修改已有订阅，必须做成默认关闭的显式配置，并在修改前保存原值或提供清晰审计记录。
...
| `minimum_score` | 是 | `80` | 自动接受最低总分，范围 0 到 300 |
| `minimum_margin` | 是 | `15` | 第一名最低领先分，范围 0 到 100 |
...
刷新市场后安装“豆瓣订阅助手”。确认宿主版本不低于 `2.15.1`。
...
4. **已有订阅不会被修正**
   ...
   `existing` 会永久加入成功去重。如果原订阅未锁定或总集数错误，当前插件不会再次处理它。

### 15.2 P1：建议下一阶段实现

1. 增加“仅预览，不创建订阅”的 dry-run 模式。
2. 增加人工确认候选页面，展示第一名、备选、分数和证据。
3. 增加默认关闭的“修正已有订阅”选项和审计记录。
...
1. 最终写入订阅的 `total_episode` 只能来自豆瓣明确总集数。
2. TMDB 集数只能参与候选评分。
...
1. 在测试 MoviePilot 中安装 `0.2.0`，使用一条 RSS 做端到端联调。
2. 根据真实豆瓣详情结构补充固定样本测试。
3. 根据真实 TMDB 候选结果调整评分，但不要降低安全阈值来掩盖数据问题。
4. 增加 dry-run 和人工确认能力。
5. 增加可选的已有订阅修正流程。
6. 增加速率限制、退避和通知。
7. 完成一轮回归后升级到 `0.3.0`。
...
当前功能基线是 v0.2.0。不要把 TMDB 集数写入 MoviePilot 订阅总集数；总集数必须来自豆瓣，并使用 manual_total_episode=1 锁定。不要默认修改已有订阅。先报告你对当前实现、未验证部分和准备修改范围的理解，再开始改代码。修改后运行 compileall、全部 unittest 和 git diff --check。
```

## 20. 交接结论

当前代码已经形成一个可安装的 MoviePilot V2 插件初版，核心安全策略和纯逻辑测试已完成，公开仓库也已经发布。下一阶段的首要工作不是继续堆功能，而是在真实 MoviePilot 环境中用单条 RSS 完成订阅创建、季度选择、豆瓣总集数和锁定持久化的端到端验证；验证结果应补回本文档和自动化测试。
