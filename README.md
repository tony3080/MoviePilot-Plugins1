...
面向 MoviePilot V2 的第三方插件仓库。插件从用户配置的 RSS，以及猫眼全网电视剧和网剧热度榜获取电视剧，按国家或地区过滤，读取豆瓣声明的总集数并匹配 TMDB。已经开播但豆瓣尚未公布总集数时，会暂按 100 集订阅并按执行周期持续复核；受管订阅完成后会暂停卡片，延迟复核豆瓣总集数，没有增加时由 MoviePilot 正常完成。插件还会每天记录当日有 TMDB 排期的活动电视剧订阅，晚间对进度没有增加的订阅触发缺集搜索。

当前版本：`0.5.2`，要求 MoviePilot `2.15.0` 或更高版本。

本地开发交接文档记录了当前实现、MoviePilot 调用契约、测试状态和已知限制，不随公开仓库发布。

0.5.2 优先使用豆瓣 IMDb ID 反查 TMDB；IMDb 无结果时回退标题评分并保留诊断。精确标题、年份与多名演员组成强身份组合时可达到固定可信线，同时恢复可配置的 MoviePilot 订阅成功通知。
