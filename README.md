...
当前版本：`0.4.0`，要求 MoviePilot `2.15.0` 或更高版本。

后续开发请先阅读 [开发交接文档](./开发交接文档.md)，其中记录了当前实现、MoviePilot 调用契约、测试状态、已知限制和推荐开发顺序。

0.4.0 发布后确认的订阅历史查重、RSS 记录搜索/失败筛选/单条重试、TMDB 搜索诊断及“XXXX篇”分季匹配需求，见 [0.4.1 改进需求](./0.4.1改进需求.md)。
...
完整的豆瓣、TMDB 与订阅联调需要将仓库接入 MoviePilot V2 宿主；仓库测试覆盖 RSS 解析、地区分类、标题与季度拆分、匹配评分、豆瓣总集数复核流程、每日更新快照与补齐搜索、全历史搜索、永久去重及订阅锁定调用契约。
