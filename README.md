...
面向 MoviePilot V2 的第三方插件仓库。插件从用户配置的 RSS 获取电视剧，按国家或地区过滤，读取豆瓣声明的总集数并匹配 TMDB。已经开播但豆瓣尚未公布总集数时，会暂按 100 集订阅并按执行周期持续复核；受管订阅完成后会暂停卡片，延迟复核豆瓣总集数，没有增加时由 MoviePilot 正常完成。

当前版本：`0.3.1`，要求 MoviePilot `2.15.0` 或更高版本。
...
│   ├── test_lifecycle.py                # 模拟豆瓣总集数复核、历史搜索和永久去重
...
完整的豆瓣、TMDB 与订阅联调需要将仓库接入 MoviePilot V2 宿主；仓库测试覆盖 RSS 解析、地区分类、标题与季度拆分、匹配评分、豆瓣总集数复核流程、全历史搜索、永久去重及订阅锁定调用契约。
